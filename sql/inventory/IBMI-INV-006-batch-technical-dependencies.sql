...
ORDER BY JOB_TYPE, AUTHORIZATION_NAME, JOB_NAME;
...
SELECT
    ...
    MIN(JOB_TYPE) AS SAMPLE_JOB_TYPE,
    MIN(SUBSYSTEM) AS SAMPLE_SUBSYSTEM
FROM TABLE(QSYS2.ACTIVE_JOB_INFO()) AS A
WHERE JOB_TYPE <> 'INTERACTIVE'
...
SELECT
    ...
    LISTAGG(DISTINCT USER_PROFILE_FOR_SUBMITTED_JOB, ', ')
        WITHIN GROUP (ORDER BY USER_PROFILE_FOR_SUBMITTED_JOB) AS RUN_AS_USERS
FROM QSYS2.SCHEDULED_JOB_INFO
WHERE JOB_QUEUE_NAME IS NOT NULL
GROUP BY
